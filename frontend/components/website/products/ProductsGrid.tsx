        ...
        <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 -mx-6 px-6 lg:mx-0 lg:px-0 lg:sticky lg:top-28">
          {CATEGORIES.map((c) => (
            <button
              ...
              className={`shrink-0 text-left px-4 py-2.5 rounded-full lg:rounded-none text-[12px] tracking-[0.1em] uppercase border-b lg:border-b transition-colors whitespace-nowrap ${
                category === c.value
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-10">
            {filtered.map((product, i) => {
              const gallery = (product.item_images || []).map((img) => ({
              ...
              return (
                <FadeIn key={product.id} delay={(i % 3) * 0.08}>
                  ...
                  <ZoomableImage
                    ...
                    alt={product.title}
                    gallery={gallery.length > 0 ? gallery : [{ src: gallery[0]?.src || "/logo.png", alt: product.title }]}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      {product.description && (
                        <p className="text-xs text-[var(--ds-ink-soft)] mt-1.5 line-clamp-2">{product.description}</p>
                      )}
                    </div>
                    {!product.is_in_stock && (
                      <span className="shrink-0 text-[10px] tracking-wide uppercase text-[var(--ds-ink-soft)] border border-[var(--ds-border)] rounded-full px-2 py-0.5 mt-0.5">
                        Made to order
                      </span>
                    )}
                  </div>
                  {gallery.length > 1 && (
                    <p className="text-[10px] text-[var(--ds-ink-soft)] mt-1">{gallery.length} photos · double-click to view all</p>
                  )}
                  <Link
